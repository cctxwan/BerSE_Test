...
//                drawCubicBezier(dataSet);
     ...
     * @param dataSet
     */
    protected void drawBezier2(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            //主要解决方法在这里
            //当x轴的最小值设置默认为0时，给0+1（只有当绘制贝塞尔曲线的时候才会进到这里来，所以当绘制前的下凹时，执行这里）
            //循环遍历x轴13个坐标，使其在绘制时全部+1，已达到不下凹的情况
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            Entry cur = dataSet.getEntryForIndex(Math.max(firstIndex - 5, 0));
            Entry next = cur;
            int nextIndex = -1;
...
//                if(cur.getY() < 0 || prev.getY() < 0) continue;
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                // get the color that is set for this line_left-segment
